...
이번 실습에서는 학습을 마친 질의 응답 모델을 가지고 웹 서비스(web service)를 만들어보려고 합니다. 대강의 개념도는 그림1과 같습니다. 쿼리를 받아 답변하는 웹 서비스인데요. 쿼리를 토큰화한 뒤 모델 입력값으로 만들고 이를 모델에 태워 확률값을 계산하게 만듭니다. 이후 약간의 후처리 과정을 거쳐 답변(answer)을 하게 만드는 방식입니다.
