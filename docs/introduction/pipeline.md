...
모델 학습 및 인퍼런스에 이르는 전체 파이프라인을 소개합니다. 이 파이프라인은 본 사이트에서 소개하는 태스크(문서 분류, 개체명 인식, 질의/응답, 문서 검색, 문장 생성)에 상관 없이 공통적으로 적용됩니다.
...
인퍼런스(inference)란 학습을 마친 모델에 실제 입력을 주어서 출력값을 얻어내는 과정 전반을 일컫는 말입니다. 이 책에서는 자연어 처리 모델을 가지고 튜토리얼을 수행할 계획이므로 전반적인 인퍼런스 절차는 그림4처럼 될 것 같습니다.

## **그림4** inference
{: .no_toc .text-delta }
<img src="https://i.imgur.com/I4lGm3J.jpg" width="600px" title="source: imgur.com" />

우선 사용자의 자연어 입력(query)를 받아 이를 토큰화합니다. 이렇게 만든 토큰 시퀀스를 모델에 입력해 해당 입력에 관한 확률값을 얻어냅니다. 이 확률값들을 적당히 후처리해 자연어 형태의 출력을 만들어 답변으로 최종 리턴합니다. 이 책에서는 쿼리에서 답변에 이르는 전 과정을 웹 페이지에서 수행할 수 있는 데모 형태로 제공할 계획입니다.
