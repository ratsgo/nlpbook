...
표4는 바이그램 쌍을 키값으로 해서 표3의 빈도를 합쳐준 결과입니다.
...
더 이상 병합 대상이 존재하지 않으므로 병합을 그만둡니다. 그 다음으로는 `b`, `ug`가 각각 어휘 집합(`vocab.json`)에 있는지를 검사합니다. 둘 모두 있으므로 `bug`의 토큰화 최종 결과로 `b`, `ug`를 반환합니다.
